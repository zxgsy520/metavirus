...
def run_obtain_taxonomy(m6_file, taxonomy):

    reads_dict = {}
    blast_dict = best_blast(m6_file)
    #output = open('%s.species_annotation.txt' % name, 'w')
    
    print("#Seq id\tTax id\tSpecies\tAnnotate")
    ...
    for line in read_tsv(taxonomy):
        if line[0] in reads_dict:
            for i in reads_dict[line[0]]:
                ...
                print('%s\t%s\t%s\t%s' % (i, line[0], line[1], line[2]))
        else:
            LOG.info('%s does not have corresponding species classification information' % line[0])
    #output.close()
    return 0
...
def add_args(parser):
    ...
    parser.add_argument('input', metavar='m6', type=str,
        help='Input balst comparison file.')
    parser.add_argument('-t', '--taxonomy', metavar='FILE', type=str,
        default= '/export2/master2/sunzy/software/species.taxonomy',
...
attention:
    obtain_taxonomy.py txt.fa.m6 >species_annotation.txt
    obtain_taxonomy.py txt.fa.m6 -t /export2/master2/sunzy/software/species.taxonomy >species_annotation.txt
''')
    args = add_args(parser).parse_args()

    run_obtain_taxonomy(args.input, args.taxonomy)
